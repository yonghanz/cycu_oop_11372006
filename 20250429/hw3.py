...
def plot_station_with_icon(stop_id, stop_name, latitude, longitude, icon_path='火柴人.png', output_file='station_map.html'):
    """
    根據車站資訊繪製地圖，顯示該車站的位置，並使用自定義圖示。

    Args:
        stop_id (str): 車站號碼。
        stop_name (str): 車站名稱。
        latitude (float): 車站緯度。
        longitude (float): 車站經度。
        icon_path (str): 自定義圖示的檔案路徑。
        output_file (str): 輸出的地圖 HTML 檔案名稱。
    """
    # 繪製地圖
    ...
    folium.Marker(
        location=map_center,
        popup=f"車站名稱: {stop_name}<br>車站號碼: {stop_id}",
        tooltip=stop_name,
        icon=icon
    ).add_to(m)
...
if __name__ == "__main__":
    ...
    # 詢問使用者輸入車站號碼
    stop_id_input = input("請輸入車站號碼：")

    # 查詢車站資訊
    try:
        stop_data = route_info.dataframe[route_info.dataframe['stop_number'] == stop_id_input]
        if stop_data.empty:
            print(f"找不到車站號碼為 {stop_id_input} 的資料。")
        else:
            ...
            # 繪製地圖
            plot_station_with_icon(
                stop_id=stop_id_input,
                stop_name=stop_name,
                latitude=latitude,
                longitude=longitude,
                icon_path='火柴人.png',
                output_file=f'station_{stop_id_input}_map.html'
            )
    except Exception as e:
        print(f"發生錯誤：{e}")