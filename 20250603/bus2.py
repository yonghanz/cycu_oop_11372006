...
if __name__ == "__main__":
    ...
    direction = input("請輸入方向（go 或 come）: ").strip().lower()

    # 驗證方向輸入
    if direction not in ['go', 'come']:
        print("❌ 輸入的方向無效，請輸入 'go' 或 'come'")
    else:
        # 初始化 taipei_route_info 並匯出站點資料
        route_info = taipei_route_info(route_id=bus_id, direction=direction)
        
        # 指定輸出 CSV 檔案路徑
        output_csv_path = f"c:/Users/User/Desktop/cycu_oop_11372006/20250603/{bus_id}_{direction}_stations.csv"
